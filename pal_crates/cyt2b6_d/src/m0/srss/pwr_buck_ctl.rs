...
#[doc = "Field `BUCK_EN` reader - Master enable for buck converter. This register is only reset by XRES, HIBERNATE wakeup, or supply supervision reset."]
pub type BUCK_EN_R = crate::BitReader<bool>;
#[doc = "Field `BUCK_EN` writer - Master enable for buck converter. This register is only reset by XRES, HIBERNATE wakeup, or supply supervision reset."]
pub type BUCK_EN_W<'a, const O: u8> = crate::BitWriter<'a, u32, PWR_BUCK_CTL_SPEC, bool, O>;
#[doc = "Field `BUCK_OUT1_EN` reader - Enable for vccbuck1 output. The value in this register is ignored unless PWR_BUCK_CTL.BUCK_EN==1. This register is only reset by XRES, HIBERNATE wakeup, or supply supervision reset. The regulator takes up to 600us to charge the external capacitor. If there is additional load current while charging, this will increase the startup time. The TRM specifies the required sequence when transitioning vccd from the LDO to SIMO Buck output #1."]
...
impl R {
    ...
    #[doc = "Bit 30 - Master enable for buck converter. This register is only reset by XRES, HIBERNATE wakeup, or supply supervision reset."]
    #[inline(always)]
    pub fn buck_en(&self) -> BUCK_EN_R {
        BUCK_EN_R::new(((self.bits >> 30) & 1) != 0)
    }
    #[doc = "Bit 31 - Enable for vccbuck1 output. The value in this register is ignored unless PWR_BUCK_CTL.BUCK_EN==1. This register is only reset by XRES, HIBERNATE wakeup, or supply supervision reset. The regulator takes up to 600us to charge the external capacitor. If there is additional load current while charging, this will increase the startup time. The TRM specifies the required sequence when transitioning vccd from the LDO to SIMO Buck output #1."]
    #[inline(always)]
