...
pub enum LFCLK_SEL_A {
    ...
    ILO1 = 4,
    #[doc = "5: ECO_PRESCALE - External-Crystal Oscillator after prescaling in CLK_ECO_PRESCALE. Does not work in DEEPSLEEP or HIBERNATE modes. Intended for applications that operate in ACTIVE/SLEEP modes only. This option is only valid when an ECO present in the product."]
    ECO_PRESCALE = 5,
}
...
impl LFCLK_SEL_R {
    ...
    pub fn variant(&self) -> Option<LFCLK_SEL_A> {
        match self.bits {
            ...
            5 => Some(LFCLK_SEL_A::ECO_PRESCALE),
            _ => None,
        }
    }
    ...
    #[doc = "Checks if the value of the field is `ECO_PRESCALE`"]
    #[inline(always)]
    pub fn is_eco_prescale(&self) -> bool {
        *self == LFCLK_SEL_A::ECO_PRESCALE
    }
}
...
impl<'a, const O: u8> LFCLK_SEL_W<'a, O> {
    ...
    #[doc = "ECO_PRESCALE - External-Crystal Oscillator after prescaling in CLK_ECO_PRESCALE. Does not work in DEEPSLEEP or HIBERNATE modes. Intended for applications that operate in ACTIVE/SLEEP modes only. This option is only valid when an ECO present in the product."]
    #[inline(always)]
    pub fn eco_prescale(self) -> &'a mut W {
        self.variant(LFCLK_SEL_A::ECO_PRESCALE)
    }
}
