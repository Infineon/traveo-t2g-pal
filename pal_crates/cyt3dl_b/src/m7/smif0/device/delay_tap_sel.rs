...
#[doc = "Field `SEL` reader - Delay line tap selection in output / feedback clock based capture scheme (CLOCK_IF_RX_SEL = \\[0..3\\]) and RWDS capture scheme (CLOCK_IF_RX_SEL = \\[6..7\\]). This is used to shift the strobe signal into the data eye. Any 2 devices connected in a dual quad SPI arrangement must set this register equivalently. Note: DELAY_TAP_SEL must not be changed while STATUS.BUSY=1."]
pub type SEL_R = crate::FieldReader<u8, u8>;
#[doc = "Field `SEL` writer - Delay line tap selection in output / feedback clock based capture scheme (CLOCK_IF_RX_SEL = \\[0..3\\]) and RWDS capture scheme (CLOCK_IF_RX_SEL = \\[6..7\\]). This is used to shift the strobe signal into the data eye. Any 2 devices connected in a dual quad SPI arrangement must set this register equivalently. Note: DELAY_TAP_SEL must not be changed while STATUS.BUSY=1."]
