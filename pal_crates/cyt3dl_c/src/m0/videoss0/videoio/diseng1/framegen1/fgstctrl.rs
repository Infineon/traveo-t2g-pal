...
#[doc = "Field `FGSYNCMODE` reader - Determines the operating mode of the framegen unit for side-by-side synchronization. The framegen unit programmed as master generates synchronization pulses. The framegen units programmed as slaves are synchronized to the master unit. There must be only one framegen unit programmed acting as master."]
pub type FGSYNCMODE_R = crate::FieldReader<u8, FGSYNCMODE_A>;
#[doc = "Determines the operating mode of the framegen unit for side-by-side synchronization. The framegen unit programmed as master generates synchronization pulses. The framegen units programmed as slaves are synchronized to the master unit. There must be only one framegen unit programmed acting as master.\n\nValue on reset: 0"]
