...
pub type LOWER_W<'a, const O: u8> = crate::FieldWriter<'a, u32, REF_LIMIT_SPEC, u16, u16, 16, O>;
...
pub type UPPER_W<'a, const O: u8> = crate::FieldWriter<'a, u32, REF_LIMIT_SPEC, u16, u16, 16, O>;
impl R {
    #[doc = "Bits 0:15 - Cycle time lower limit. Set the lower limit -1, in reference clock cycles, before the next monitored clock event is allowed to happen. If a monitored clock event happens before this limit is reached a CSV error is detected. LOWER must be at least 1 less than UPPER. In case the clocks are asynchronous LOWER must be at least 3 less than UPPER."]
    #[inline(always)]
    pub fn lower(&self) -> LOWER_R {
        LOWER_R::new((self.bits & 0xffff) as u16)
    }
    #[doc = "Bits 16:31 - Cycle time upper limit. Set the upper limit -1, in reference clock cycles, before (or same time) the next monitored clock event must happen. If a monitored clock event does not happen before this limit is reached, or does not happen at all (clock loss), a CSV error is detected."]
    #[inline(always)]
