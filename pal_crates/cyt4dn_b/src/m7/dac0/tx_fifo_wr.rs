...
#[doc = "Field `DATA` writer - Data written to the TX FIFO. Writing adds the data to the TX FIFO; i.e. behavior is similar to that of a PUSH operation (TX_FIFO_STATUS.WR_PTR is incremented and TX_FIFO_STATUS.USED is incremented). -16-bit left PCM data = DATA\\[31:16\\]
-16-bit right PCM data = DATA\\[15:0\\]
...
impl W {
    #[doc = "Bits 0:31 - Data written to the TX FIFO. Writing adds the data to the TX FIFO; i.e. behavior is similar to that of a PUSH operation (TX_FIFO_STATUS.WR_PTR is incremented and TX_FIFO_STATUS.USED is incremented). -16-bit left PCM data = DATA\\[31:16\\]
-16-bit right PCM data = DATA\\[15:0\\]
Note: Writing to a full TX FIFO activates INTR.TX_FIFO_OVERFLOW."]
    #[inline(always)]
    #[must_use]
