...
#[doc = "Field `PERFRESULT` reader - Returns the performance counter value. Returns number of cycles of the last frame on the input. To calculate the performance divide the known number of pixels of the frame by this number. For debug purposes only, read when stable only, otherwise read data might be corrupted."]
pub type PERFRESULT_R = crate::FieldReader<u32, u32>;
impl R {
    #[doc = "Bits 0:31 - Returns the performance counter value. Returns number of cycles of the last frame on the input. To calculate the performance divide the known number of pixels of the frame by this number. For debug purposes only, read when stable only, otherwise read data might be corrupted."]
    #[inline(always)]
