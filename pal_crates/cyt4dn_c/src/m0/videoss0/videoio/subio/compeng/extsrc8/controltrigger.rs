...
#[doc = "Field `SHDTOKGEN` writer - Write a 1 to this field for the extsrc unit to perform a shadow load with the next start of operation and set the rld bit in the start of frame control word (generate shadow load token for subsequent units)."]
pub type SHDTOKGEN_W<'a, const O: u8> = crate::BitWriter<'a, u32, CONTROLTRIGGER_SPEC, bool, O>;
impl W {
    #[doc = "Bit 0 - Write a 1 to this field for the extsrc unit to perform a shadow load with the next start of operation and set the rld bit in the start of frame control word (generate shadow load token for subsequent units)."]
    #[inline(always)]
    #[must_use]
