...
#[doc = "Field `SPLITPOSITION` reader - Index of first column of right display half when ChannelMode is DUAL_SPLIT. SplitPosition and (Hact - SplitPosition) must be less or equal than max supported active frame width. If SplitPosition is greater than (Hact - SplitPosition), then Htotal must be greater than 2*SplitPosition; else Htotal must be greater than (Hact - SplitPosition). Once setting a SplitPosition, data_en is needed. The SplitPosition must be even."]
pub type SPLITPOSITION_R = crate::FieldReader<u16, u16>;
#[doc = "Field `SPLITPOSITION` writer - Index of first column of right display half when ChannelMode is DUAL_SPLIT. SplitPosition and (Hact - SplitPosition) must be less or equal than max supported active frame width. If SplitPosition is greater than (Hact - SplitPosition), then Htotal must be greater than 2*SplitPosition; else Htotal must be greater than (Hact - SplitPosition). Once setting a SplitPosition, data_en is needed. The SplitPosition must be even."]
...
impl R {
    ...
    #[doc = "Bits 16:29 - Index of first column of right display half when ChannelMode is DUAL_SPLIT. SplitPosition and (Hact - SplitPosition) must be less or equal than max supported active frame width. If SplitPosition is greater than (Hact - SplitPosition), then Htotal must be greater than 2*SplitPosition; else Htotal must be greater than (Hact - SplitPosition). Once setting a SplitPosition, data_en is needed. The SplitPosition must be even."]
    #[inline(always)]
