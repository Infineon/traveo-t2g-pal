...
pub type IRQ_ENABLE_W<'a, const O: u8> = crate::FieldWriter<'a, u32, IRQ_ENABLE_SPEC, u8, u8, 8, O>;
impl R {
    #[doc = "Bits 0:7 - IRQ mask setting. Each bit in IRQ_ENABLE corresponds to each bit in IRQ_STATUS. Setting a bit in IRQ_ENABLE to 1 will prevent the corresponding bit in IRQ_STATUS from causing MIPI Rx Controller interrupt output to assert. Note: Bits 5 and 6 do not have any meaning."]
    #[inline(always)]
    pub fn irq_enable(&self) -> IRQ_ENABLE_R {
        IRQ_ENABLE_R::new((self.bits & 0xff) as u8)
    }
}
impl W {
    #[doc = "Bits 0:7 - IRQ mask setting. Each bit in IRQ_ENABLE corresponds to each bit in IRQ_STATUS. Setting a bit in IRQ_ENABLE to 1 will prevent the corresponding bit in IRQ_STATUS from causing MIPI Rx Controller interrupt output to assert. Note: Bits 5 and 6 do not have any meaning."]
