...
pub enum CHANNELMODE_A {
    ...
    DUAL_INTERLEAVED = 1,
    #[doc = "2: Dual pixel mode. Both channels are active at half the pixel clock. 1st channel drives the left and 2nd one the right half of the display. Note data_en is needed in this mode"]
    DUAL_SPLIT = 2,
}
...
impl<'a, const O: u8> CHANNELMODE_W<'a, O> {
    ...
    #[doc = "Dual pixel mode. Both channels are active at half the pixel clock. 1st channel drives the left and 2nd one the right half of the display. Note data_en is needed in this mode"]
    #[inline(always)]
...
pub enum LVDSMODE_A {
    #[doc = "1: MiniLVDS"]
    MINI_LVDS = 1,
    #[doc = "0: LVDS Mode, refer to OpenLDI"]
    LVDS = 0,
}
...
impl<'a, const O: u8> LVDSMODE_W<'a, O> {
    ...
    #[doc = "LVDS Mode, refer to OpenLDI"]
    #[inline(always)]
...
#[doc = "Field `LVDS_CLOCK_INV` reader - Inversion the polarity of lvds clock in OpenLDI Mode"]
pub type LVDS_CLOCK_INV_R = crate::BitReader<LVDS_CLOCK_INV_A>;
#[doc = "Inversion the polarity of lvds clock in OpenLDI Mode\n\nValue on reset: 0"]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
#[doc = "Field `LVDS_CLOCK_INV` writer - Inversion the polarity of lvds clock in OpenLDI Mode"]
...
#[doc = "Field `SPLITPOSITION` reader - Index of first column of right display half when ChannelMode is DUAL_SPLIT. SplitPosition and (Hact - SplitPosition) must be less or equal than half of the max supported active frame width. If SplitPosition is greater than (Hact - SplitPosition), then Htotal must be greater than 2*SplitPosition; else Htotal must be greater than 2*(Hact - SplitPosition). Once setting a SplitPosition, data_en is needed. The SplitPosition must be even."]
pub type SPLITPOSITION_R = crate::FieldReader<u16, u16>;
#[doc = "Field `SPLITPOSITION` writer - Index of first column of right display half when ChannelMode is DUAL_SPLIT. SplitPosition and (Hact - SplitPosition) must be less or equal than half of the max supported active frame width. If SplitPosition is greater than (Hact - SplitPosition), then Htotal must be greater than 2*SplitPosition; else Htotal must be greater than 2*(Hact - SplitPosition). Once setting a SplitPosition, data_en is needed. The SplitPosition must be even."]
...
impl R {
    ...
    #[doc = "Bit 11 - Inversion the polarity of lvds clock in OpenLDI Mode"]
    #[inline(always)]
    ...
    #[doc = "Bits 16:29 - Index of first column of right display half when ChannelMode is DUAL_SPLIT. SplitPosition and (Hact - SplitPosition) must be less or equal than half of the max supported active frame width. If SplitPosition is greater than (Hact - SplitPosition), then Htotal must be greater than 2*SplitPosition; else Htotal must be greater than 2*(Hact - SplitPosition). Once setting a SplitPosition, data_en is needed. The SplitPosition must be even."]
    #[inline(always)]
