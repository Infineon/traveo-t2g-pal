...
#[doc = "Field `FRAMEXOFFSET` reader - Horizontal offset (X). FrameXOffset * BitsPerPixel must be byte aligned. The same applies to the Store unit's input frame width * BitsPerPixel. (format is signed integer)"]
pub type FRAMEXOFFSET_R = crate::FieldReader<u16, u16>;
#[doc = "Field `FRAMEXOFFSET` writer - Horizontal offset (X). FrameXOffset * BitsPerPixel must be byte aligned. The same applies to the Store unit's input frame width * BitsPerPixel. (format is signed integer)"]
...
pub type FRAMEYOFFSET_W<'a, const O: u8> =
    crate::FieldWriter<'a, u32, FRAMEOFFSET_SPEC, u16, u16, 15, O>;
impl R {
    #[doc = "Bits 0:14 - Horizontal offset (X). FrameXOffset * BitsPerPixel must be byte aligned. The same applies to the Store unit's input frame width * BitsPerPixel. (format is signed integer)"]
    #[inline(always)]
