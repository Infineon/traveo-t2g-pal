...
#[doc = "Field `PHASE_OFFSET_H` reader - Phase offset has to be larger than -2 and smaller than +2. Depending on scale_mode this value is relative to input (UPSCALE) or output pixel (DOWNSCALE). A negative value shifts the image to the right, a positive one to the left. (format is signed fix-point 2.19)"]
pub type PHASE_OFFSET_H_R = crate::FieldReader<u32, u32>;
#[doc = "Field `PHASE_OFFSET_H` writer - Phase offset has to be larger than -2 and smaller than +2. Depending on scale_mode this value is relative to input (UPSCALE) or output pixel (DOWNSCALE). A negative value shifts the image to the right, a positive one to the left. (format is signed fix-point 2.19)"]
pub type PHASE_OFFSET_H_W<'a, const O: u8> =
    crate::FieldWriter<'a, u32, SCALERSETUPH1_SPEC, u32, u32, 21, O>;
impl R {
    #[doc = "Bits 0:20 - Phase offset has to be larger than -2 and smaller than +2. Depending on scale_mode this value is relative to input (UPSCALE) or output pixel (DOWNSCALE). A negative value shifts the image to the right, a positive one to the left. (format is signed fix-point 2.19)"]
    #[inline(always)]
